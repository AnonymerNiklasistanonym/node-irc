    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'rpl_whoischannels':
               // TODO - clean this up?
                // FIX TYPE ERROR THAT CRASHES THE WHOLE APPLICATION
                self._addWhoisData(message.args[1], 'channels', message.args[2] !== undefined ? message.args[2].trim().split(/\s+/) : []);
                break;
...
Client.prototype.connect = function(retryCount, callback) {
    ...
    function handleData(chunk) {
        // FIX CRITICAL ERROR THAT CRASHES THE WHOLE APPLICATION
        if (self.conn == null) {
            return;
        }
